...
async function extractStreamUrl(html) {
  ...
  for (const srv of servers) {
    try {
      ...
      const vidMatch = pageHtml.match(/<video[^>]*\s+src=["']([^"']+)["']/i);
      ...
      console.log(`Failed to fetch ${srv.url} for ${srv.name}`);
    }
  }
  ...
  if (!results.length) {
    return JSON.stringify({
      status: 'error',
      message: 'لم يتم استخراج أي فيديو من السيرفرات',
      url: fallback
    });
  }
